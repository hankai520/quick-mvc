    ...
    if (files != null) {
      for (File f : files) {
        ...
        try {
          ...
          logger.warn(String.format("Failed to calculate checksum of %s", f.getAbsolutePath()), e);
        }
        ...
        try {
          ...
          logger.error(String.format("Failed to parse timestamp from file %s", f.getAbsolutePath()),
              e);
        }
        list.add(b);
      }
