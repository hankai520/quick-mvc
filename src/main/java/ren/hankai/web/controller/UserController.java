...
public class UserController extends AbstractController {
   ...
   */
  @RequestMapping(value = Route.BG_LOGIN, method = {RequestMethod.POST, RequestMethod.GET})
  public ModelAndView login(
      @CookieValue(value = WebConfig.COOKIE_KEY_LOGIN_ID, required = false) String loginId,
      @CookieValue(value = WebConfig.COOKIE_KEY_PASSWORD, required = false) String password,
      @ModelAttribute("user") @Valid UserViewModel user, BindingResult br,
    ...
    if ("GET".equals(method) && !user.hasContents()) {
      ...
      if (!br.hasErrors()) {
        User localUser = userService.findOne(EntitySpecs.field("mobile", user.getLoginId()));
        if (localUser == null) {
  ...
  @RequestMapping(value = Route.BG_USERS_JSON, produces = {"application/json; charset=utf-8"})
  @ResponseBody
  public PaginatedList getUsersJson(HttpSession session,
    ...
    try {
      ...
      Page<User> result =
          userService.findAll(UserSpecs.bgSearch(currentUser, null, search), pageable);
      if ((result != null) && result.hasContent()) {
        for (User u : result.getContent()) {
          u.setStatusName(messageSource.getMessage(u.getStatus().i18nKey(), null, null));
  ...
  @RequestMapping(value = Route.BG_ADD_USER, method = RequestMethod.GET)
  ...
  @RequestMapping(value = Route.BG_ADD_USER, method = RequestMethod.POST)
  public ModelAndView addUser(HttpSession session, @ModelAttribute("user") @Valid User user,
      BindingResult br) {
    ModelAndView mav = new ModelAndView("admin/add_user");
    User duplicate = userService.findOne(EntitySpecs.field("mobile", user.getMobile()));
  ...
  @RequestMapping(value = Route.BG_EDIT_USER, method = RequestMethod.GET)
  public ModelAndView editUserForm(HttpSession session, @PathVariable("user_id") Integer userId) {
  ...
  @RequestMapping(value = Route.BG_EDIT_USER, method = RequestMethod.POST)
  public ModelAndView editUser(@PathVariable("user_id") Integer userId,
      @ModelAttribute("user") @Valid User user, BindingResult br, HttpSession session) {
    ModelAndView mav = new ModelAndView("admin/edit_user");
    User currentUser = WebConfig.getUserInSession(session);
  ...
  public ModelAndView deleteUser(@PathVariable("user_id") Integer userId, HttpSession session) {
  ...
  @RequestMapping(value = Route.BG_CHANGE_USER_PWD, method = RequestMethod.GET)
  public ModelAndView changePwdForm(@PathVariable("user_id") Integer userId, HttpSession session) {
    ...
    if (user == null) {
      mav.setViewName("redirect:/404.html");
        ...
        && (currentUser.getRole() != UserRole.SuperAdmin) && !currentUser.getId().equals(userId)) {
      mav.setViewName("redirect:" + Route.BG_USERS);
    } else {
      mav.addObject("user", user);
    }
  ...
  @RequestMapping(value = Route.BG_CHANGE_USER_PWD, method = RequestMethod.POST)
  public ModelAndView changePwd(@PathVariable("user_id") Integer userId,
